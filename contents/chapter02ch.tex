...
地質學藉由地表觀察，可得知過去地表上經歷的地質事件，可間接獲得當時的動力狀態，不過隨著時間推移，可用的岩石紀錄越來越少，並且地質資料並無法完全代表地球內部的動力過程。
...
\begin{figure*}[ht!]
    ...
    \caption{鎂鐵質岩相圖，綠線隔開玄武岩與榴輝岩的穩定場，改編自\citealp{Hacker2003}，橘線為鎂鐵質岩的含水固相線(wet solidus)，改編自\citealp{Gutscher2000Bcan}}
...
P >  0.7 \;GPa 
\end{align}
本研究有考量沉積物的部分熔融，參考自\citealp{Forster2021}。
\begin{align}
...
\begin{figure*}[ht!]
    ...
    \caption{沉積物變質岩相圖，橘線為沉積物含水固相線，改編自\citealp{Forster2021}。}
    \label{fig::sediment_phase_diagram}
\end{figure*}

\subsection{綠泥石 --- 橄欖石}

模型中的地函除了普通橄欖石與蛇紋化橄欖石之外，還有考慮綠泥石的存在。
本研究假設部分熔融事件只會發生在具有綠泥石之海洋岩石圈上方的地函楔。
一旦溫度太高而使岩石脫水，綠泥石就會轉變為一般的橄欖石。
相變條件式參考\citealp{Grove2009}，簡化式如下:
\begin{align}
T > 800-35\times 10^{-9}\times (depth-62)^{2 \;\circ}C
\end{align}
圖\ref{fig::chlorite_phase_diagram}為綠泥石岩相圖。
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=4in]{chlorite_phase_diagram_v1.pdf}
    \caption{綠泥石岩相圖，橘紅線為綠泥石脫水相變線，參考自\citealp{Grove2009}。}
...
\end{align}
$T$ 是溫度，$T_m$ 是地函溫度，在本研究中使用1603K(克爾文)。
...
\begin{figure*}[h!]
    \centering
    \includegraphics[width=6in]{temperature_profile_v1.pdf}
...
岩漿量的變化意味著岩石的物態改變。
物質的物態改變在溫度相同的情形下會有能量的吸收或釋放，此時需要考慮岩漿量變化過程中的潛熱以滿足能量守恆。
